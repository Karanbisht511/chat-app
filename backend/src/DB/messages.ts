...
export const saveMessageToDB = async (index: string, message: string,sender:string,isFile?:boolean,fileMetaData?:IfileMetaData) => {
...
console.log('metadata:',fileMetaData);
        ...
        await Messages.findOneAndUpdate({ index }, { '$push': { 'messageArray': { timeStamp: new Date(), msg: message,msgBy:sender,isFile,fileMetaData } } })
        return
    } catch (error) {
