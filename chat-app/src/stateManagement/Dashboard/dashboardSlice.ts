...
export const addNewChat = createAsyncThunk(
    ...
    async (friend: string) => {
        const username = sessionStorage.getItem('username');
        const res = await axios.post(`http://localhost:9000/api/friends/addFriend?username=${username}`,
            {
                "frToAdd": friend
            },
            {
...
export const deleteChat = createAsyncThunk(
    ...
    async (friend: string) => {
        const username = sessionStorage.getItem('username');
        const res = await axios.post(`http://localhost:9000/api/friends/deleteFriend?username=${username}`,
            {
                "frToDelete": friend
            },
            {
