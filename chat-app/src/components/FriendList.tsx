...
import { toggleNewChat, toggleAddGroupMember, toggleCreateGroupPopup } from '../stateManagement/PopupContexts/PopupContext';
...
const FriendList: React.FC<PList> = ({ friendList, users, groups }) => {
  ...
  const handlePopup = () => {
    ...
    if (showNewChatPopup === false) {
      ...
      if (showAddGroupMemberPopup)
        dispatch(toggleAddGroupMember())
  ...
  return (
    <div className='chatList h-full'>
      <div className='user-name flex justify-between'><div><span>Chats</span></div><div> <button onClick={handlePopup}> <FontAwesomeIcon icon={faPenToSquare} /> </button></div></div>
